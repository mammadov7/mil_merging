...
def ties_merge_key(fine_tuned_models, init_model, k=1.0, lam=1.0):
...
def get_bag_feats(csv_file_df, args, random_state=1):
    ...
    # feats = df.reset_index(drop=True) # COMMENT AFTER STD EXPERIMENT
...
def get_model(args):
    if args.model == 'maxmil':
        if args.num_classes > 1:
            return mils.mcmaxmil(input_size=args.feats_size, output_size=args.num_classes)
        else:
            return mils.maxmil(input_size=args.feats_size, output_size=1,init_type=args.init_type)

    elif args.model == 'abmil':
        if args.num_classes > 1:
            return abmil.mcabmil(input_size=args.feats_size, output_size=args.num_classes)
        else:
            return abmil.abmil(input_size=args.feats_size)

    elif args.model == 'softmil':
        return mils.softmil(input_size=args.feats_size, output_size=1)

    elif args.model == 'meanmil':
        if args.num_classes > 1:
            return mils.mcmeanmil(input_size=args.feats_size, output_size=args.num_classes)
        else:
            return mils.meanmil(input_size=args.feats_size, output_size=args.num_classes)

    elif args.model == 'automil':
        if args.num_classes > 1:
            return mils.mcautomil(input_size=args.feats_size, output_size=args.num_classes, alpha=1.)
        else:
            return mils.automil(input_size=args.feats_size, output_size=1, alpha=5.)

    elif args.model == 'clam':
        if args.num_classes > 1:
            return CLAM_SB(input_size=args.feats_size, n_classes=args.num_classes)
        else:
            return CLAM_SB(input_size=args.feats_size)
    
    elif args.model == 'lnpmil':
        if args.num_classes > 1:
            return mils.mclnpmil(input_size=args.feats_size, output_size=args.num_classes, norm=1.)
        else:
            return mils.lnpmil(input_size=args.feats_size, output_size=1, norm=1.)

    elif args.model == 'mixmil':
        if args.num_classes > 1:
            return mils.mcmixmil(input_size=args.feats_size, output_size=args.num_classes, alpha=1.)
        else:
            return mils.mixmil(input_size=args.feats_size, output_size=1, alpha=1.)

    elif args.model == 'attenmil':
        if args.num_classes > 1:
            return mils.mcattenmil(input_size=args.feats_size, output_size=args.num_classes)
        else:
            return mils.attenmil(input_size=args.feats_size, output_size=1)

    else:
        print("No model named "+model)
        return -1

def print_parameter(args,model):
    if args.model == 'automil':
        print( model.alpha)
    elif args.model == 'mixmil':
        print( model.alpha)
    elif args.model == 'lnpmil':
        print( model.p)
    elif args.model == 'lsemil':
...
def main():
    ...
    parser.add_argument('--init_seed', default=1, type=int, help='Seed for shuffle')
    ...
    parser.add_argument('--init_type', default='orthogonal', type=str, help='uniform, xavier, switch')
    ...
    early_aucs=[]
    M, N = len(mil_ids), args.nb_epochs
    ...
    ema_early_auc = np.average(early_aucs, weights=[1, 1, 1, 1, 1], axis=1)
    # ema_early_auc = np.average(early_aucs, axis=1)
    ...
    for epoch in range(1, args.num_epochs):
        patience+=1
        random_state = np.random.RandomState(seeds[bests[(epoch-1)%3]])
        train_path = shuffle(train_path, random_state=random_state).reset_index(drop=True)
        val_path = shuffle(val_path, random_state=random_state).reset_index(drop=True)
